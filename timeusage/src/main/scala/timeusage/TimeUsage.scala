...
object TimeUsage {
  ...
  def row(line: List[String]): Row = {
    val col1 = line.head.toString
    val rest = line.tail.map(_.toDouble)
    Row.fromSeq(col1 :: rest)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primary_prefixes = List("t01", "t03", "t11", "t1801", "t1803")
  ...
  def timeUsageSummary(
    ...
    val sexProjection: Column = when($"tesex" === 1.0, "male").otherwise("female")
      .as("sex")
